...
class EncoderLayer(nn.Module, AdapterLayerMix):
    ...
    def forward(self, x, encoder_padding_mask, output_attentions=False):
        ...
        if self.add_adapter_self_attn:
            x = self.adapter_self_attn_forward(x, x)
            x = residual + x
        ...
        if self.add_adapter_ffn:
            x = self.adapter_ffn_forward(x, x)
            x = residual + x
...
class BartEncoder(nn.Module, MixAdapterEncDec):
    """
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        if config.static_position_embeddings:
            self.embed_positions = SinusoidalPositionalEmbedding(
                config.max_position_embeddings, embed_dim, self.paadding_idx
            )
        else:
        ...
        # adapter stuff
        MixAdapterEncDec.__init__(self)
    ...
    def forward(
        ...
        if self.add_adapter_tok_embed:
            x = self.adapter_tok_embed_forward(x, x)
...
class DecoderLayer(nn.Module, AdapterLayerMix):
    def __init__(self, config: BartConfig):
        super().__init__()
        self.embed_dim = config.d_model
    ...
    def forward(
        ...
        if self.add_adapter_cross_attn:
            x = self.adapter_cross_attn_forward(x, x)
            x = residual + x
...
class BartDecoder(nn.Module, MixAdapterEncDec):
    """
...
class BartForConditionalGeneration(PretrainedBartModel, MixAdapterBFCG, MixTrainableController):
    base_model_prefix = "model"
